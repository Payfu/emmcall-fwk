...
class GetSetController
{
  ...
  */
  public function updateEntity(bool $debug = false, $arrayData = []){
    $t = $this->'.$this->_nom_table_format.';
    // Si le getEntity n\'est pas lancé
    if(!$this->_selectFields){ return false; }
    
    // Si $this->__id n\'existe pas
    if(!isset($this->__id) && empty($arrayData)){
      die("Il n\'y a pas de champs id auto-incrémenté, il faut donc remplacer <pre>updateEntity()</pre> par <pre>updateEntity(false, [\'ch1\'=>value, \'ch2\'=>value])</pre> ");
    } else if(isset($this->__id) && empty($arrayData)) {
