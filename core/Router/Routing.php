...
class Routing extends YamlParser
{
  ...
  public function __construct( string $fileName, ?string $url = null ){
    parent::__construct($fileName); // constructe de YamlParser
    // On récupère l'url
    $this->_url = $url;
  ...
  private function checkParams() {
    ...
    /**
      * Etape 1
      * Gestion du PATH
      * On vérifie l'ensemble des routes et des infos indiquées
      */
    ...
    foreach($arrayRoutes as $k => $v){		
      ...
      if(strtolower($method) === "get")   { $laRoute = $router->get($path, $controller); } 
      if(strtolower($method) === "double"){ $laRoute = $router->double($path, $controller); } 
      if(strtolower($method) === "post")  { $laRoute = $router->post($path, $controller); }
